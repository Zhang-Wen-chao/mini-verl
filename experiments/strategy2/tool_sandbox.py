...
class PythonSandbox:
    ...
    def _check_code_safety(self, code: str) -> tuple[bool, str]:
        """Check executable Python syntax against the sandbox policy.

        When the SANDBOX_OFF environment variable is set (to any value),
        safety checks are skipped entirely and code runs as-is (the
        "no anti-cheat" ablation arm).
        """
        import os

        if os.environ.get("SANDBOX_OFF"):
            return True, ""
        try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                # Relative imports can escape the intended allowlist context.
                module = (node.module or "").split(".", 1)[0]
                if node.level or module not in self.allowed_modules:
                    return False, f"Import of '{module or 'relative module'}' is not allowed"
                ...
                return False, f"Dunder attribute '{node.attr}' is not allowed"

        return True, "Code is safe"

    @staticmethod
    def _instrument_final_expression(code: str) -> str:
        """Echo a non-``None`` final expression like a code interpreter.

        Tool-using policies commonly finish a calculation with a bare
        expression (for example ``comb(6, 3)``) rather than ``print``.  A
        Python script normally discards that value, making a successful tool
        call look like an empty observation.  This transforms only the final
        top-level expression into an assignment and emits its repr after user
        code runs.  The source is already AST-validated by
        :meth:`_check_code_safety`; imports, file access, reflection and
        dynamic execution remain subject to the same policy.
        """
        tree = ast.parse(code, mode="exec")
        ...
        tree.body[-1] = ast.Assign(
            targets=[ast.Name(id="_retool_last_value", ctx=ast.Store())],
        ...
        return (
            f"{transformed}\n"
            "if _retool_last_value is not None:\n"
            "    print(repr(_retool_last_value))\n"
        )
    ...
    async def execute_code(self, code: str) -> str:
        ...
        # Make bare final expressions observable before adding the execution
        # wrapper.  AST parsing cannot fail here because safety validation
        # above already parsed the same source.
        executable_code = self._instrument_final_expression(code)
        ...
        indented_code = "\n".join("    " + line for line in executable_code.split("\n"))
