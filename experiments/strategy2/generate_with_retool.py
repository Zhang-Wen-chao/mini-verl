...
async def execute_predictions(prediction: str) -> tuple[str, bool, str, str, bool]:
    """Execute one model action and return observation plus its outcome.

    The explicit outcome is retained on the sample for the quality-process
    reward. Counting a request as a tool call is not enough: an attempted
    import of ``os`` and a successful calculation need different signals.
    """
    ...
    if action == "code":
        ...
        code, was_markdown_fenced = normalize_markdown_code(raw_code)
        if code:
            async with SEMAPHORE:
                result = await tool_registry.execute_tool("code_interpreter", {"code": code})
            next_obs = f"\n\n<interpreter>\n{result}\n</interpreter>\n\n"
            done = False
            outcome = "tool_success" if tool_execution_succeeded(result) else "tool_failure"
        else:
            next_obs = "\n\n<interpreter>\nError: No Python code found" "\n</interpreter>\n\n"
            done = False
            outcome = "tool_failure"
            was_markdown_fenced = False
    elif action == "answer":
        next_obs = ""
        done = True
        outcome = "answer"
    ...
    return next_obs, done, outcome, raw_code if action == "code" else content, was_markdown_fenced
...
async def generate(args, sample: Sample, sampling_params) -> Sample:
    ...
    markdown_fenced_tool_call_count = 0
    ...
    for turn in range(TOOL_CONFIGS["max_turns"]):
        ...
        next_obs, done, action_outcome, action_content, was_markdown_fenced = await execute_predictions(cur_response)
        if action_outcome == "answer":
            submitted_answer = True
        if done:
            break
        if action_outcome == "tool_success":
            tool_success_count += 1
        elif action_outcome == "tool_failure":
            tool_failure_count += 1
        elif action_outcome == "invalid_action":
        ...
        if "<interpreter>" in next_obs:
            tool_call_count += 1
            markdown_fenced_tool_call_count += int(was_markdown_fenced)
            fingerprint = code_fingerprint(action_content)
    ...
    sample.tool_success_count = tool_success_count
    sample.tool_failure_count = tool_failure_count
    sample.invalid_action_count = invalid_action_count
    sample.repeated_tool_call_count = repeated_tool_call_count
    sample.markdown_fenced_tool_call_count = markdown_fenced_tool_call_count
    sample.submitted_answer = submitted_answer
...
async def reward_func(args, sample, **kwargs):
    """Outcome reward with optional legacy or quality-aware process terms.

    ``QUALITY_PROCESS_REWARD`` is the repaired Strategy 2 arm. Unlike legacy
    ``PROCESS_REWARD``, it rewards only successful execution and penalizes
    errors, repeats, malformed actions, and missing final answers.
    """
    import os
    ...
    if os.environ.get("QUALITY_PROCESS_REWARD"):
        result["score"] = quality_process_score(
            float(result["score"]),
            submitted_answer=bool(getattr(sample, "submitted_answer", False)),
            tool_successes=int(getattr(sample, "tool_success_count", 0)),
            tool_failures=int(getattr(sample, "tool_failure_count", 0)),
            invalid_actions=int(getattr(sample, "invalid_action_count", 0)),
            repeated_tool_calls=int(getattr(sample, "repeated_tool_call_count", 0)),
            markdown_fenced_tool_calls=int(getattr(sample, "markdown_fenced_tool_call_count", 0)),
        )
    elif os.environ.get("PROCESS_REWARD"):
        # Process-reward arm: explicit credit for tool use, independent of outcome.
        tool_reward = 0.2 * min(num_turns, 3)  # cap at 3 tool rounds
        # final-answer score is already in result["score"] (1.0 or negative)
        base = max(0.0, result["score"])  # 1.0 if correct else 0
        result["score"] = base + tool_reward
        if result["score"] == 0:
            result["score"] = -1.0  # wrong answer, no tool use
    else:
        # Original outcome-reward arm: encourage model to call tools only when wrong.
        if result["score"] < 0:
