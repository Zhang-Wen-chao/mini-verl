...
本页只说明 `mini_verl/` 这个最小 GRPO 框架：它实现了什么、刻意没有实现什么、以及
为什么这样拆分。项目当前计划看 [状态与路线图](../project-status.md)，真实 4B 结果看
[679-step 实验结果](../results/qwen3.5-4b-grpo-679-step.md)。

`mini_verl` 以 `verl` 的核心数据流为原型，但不追求 API 兼容；目标是用可读、可测、
可 profile 的最小实现，打通 rollout、奖励、GRPO 更新和分布式资源协同。
...
完整复现命令和口径见 [运行指南](../guides/runbook.md)；L20 toy CUDA 基准的环境、命令、原始 JSON 与解释见 [性能实验](../engineering/mini-verl-performance.md)。

## 与官方 verl 实验的关系

`mini_verl` 不复制 Ray、FSDP 或 vLLM 的生产编排。官方实验层用于验证同一条核心
数据流能否在真实训练栈上成立；其结果、证据与历史记录从
[官方实验资产索引](../../official_verl/README.md) 进入。
